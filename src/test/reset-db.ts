...
// Future slices that add tables must extend this list to preserve full isolation.
export async function resetDb(db: PrismaClient): Promise<void> {
  ...
  await db.$executeRawUnsafe(
    'TRUNCATE TABLE "users", "allowlist_entries", "projects", "memberships", "catalog_items", "lists", "list_items", "favorites", "absorbed_entries", "recipes", "recipe_items" RESTART IDENTITY CASCADE;'
  );
}
