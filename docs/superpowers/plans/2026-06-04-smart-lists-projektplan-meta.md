...
- **Deliberate cuts:** Plurals are recognized only when explicitly listed in `BASE_UNIT_ALIASES`; `2x Milch` is not special-cased; and the escape hatch for `7 Zwerge Bier` applies only after that exact catalog article exists, so the first-ever entry may still parse as quantity `7` plus article `Zwerge Bier`. The raw-plus-parseable create-row label for a partial input such as `7 Zwe` remains a deferred minor.
