...
- **Destructive actions confirm through a bottom sheet** (`ConfirmSheet`) that spells out the consequence. `ConfirmSheet` does not close itself: an `onSelect` fires the mutation *and* closes the sheet.
